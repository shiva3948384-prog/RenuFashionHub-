...
async function startServer() {
  ...
  // Load firebase-applet-config.json dynamically to read Firestore credentials
  ...
  const firestoreProjectId = firebaseConfig.projectId || "ai-studio-applet-webapp-644f0";
  const firestoreDatabaseId = firebaseConfig.firestoreDatabaseId || "ai-studio-06f9cdf8-bcdb-4985-98dd-f7d34d6cf66c";
  const firestoreApiKey = firebaseConfig.apiKey || "AIzaSyDoT5LpW7SWxNpEMe_7f-I8jZtuI3S-E7I";
  const firestoreBaseUrl = `https://firestore.googleapis.com/v1/projects/${firestoreProjectId}/databases/${firestoreDatabaseId}/documents`;

  // Convert Firestore values to standard JS types
  function convertValue(value: any): any {
    ...
    if ('arrayValue' in value) {
      ...
      return value.arrayValue.values.map((val: any) => convertValue(val));
    }
  ...
  app.get("/api/messages", async (req, res) => {
    try {
      ...
      console.warn("Supabase messages fetch failed (table may not exist), using local backup.");
    } catch (err) {
      // Ignore
    }
  ...
  app.post("/api/messages", async (req, res) => {
    ...
    try {
      ...
      console.warn("Supabase messages insert failed, saving to local backup.");
    } catch (err) {
      // Ignore
    }
